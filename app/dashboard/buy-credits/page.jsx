            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 py-10 gap-6 ">
                {
                    creditsOption.map((item, index) => (
                        ...
                        ${selectedOption?.credits == item.credits && 'border-primary'}`}>
                            <CardHeader>
                                <CardTitle className="flex font-bold text-3xl justify-center items-center">{item.credits} <span className="font-medium text-2xl p-3">Credits</span></CardTitle>
                            </CardHeader>
                            <CardContent>
                                <Button className='w-[100px]' onClick={() => setSelectedOption(item)}>Select</Button>
                            </CardContent>
                            <CardFooter>
                                <h2 className="font-semibold text-xl text-primary">$ {item.amount}</h2>
                            </CardFooter>
                        </Card>
                    ))
                }
